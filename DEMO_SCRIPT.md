...
"What's next, on two fronts. **Business:** our own broker license, direct ACE filing, carrier tracking, drawback automation. **Platform:** ship to Cloudflare — **Durable Objects** for agent memory, **Workflows** for a durable pipeline; turn the eval into a **CI gate**. And the north star: a **neuro-symbolic engine** that emits a **machine-checkable proof** for every classification — turning *reasonable care* into a verifiable artifact — and **reinforcement learning on real CBP outcomes**, so it learns to file correctly from the field and measurably beats a human broker. The work of a customs broker — done by AI, certified by a human. **That's Customs Agent Suite.**"
