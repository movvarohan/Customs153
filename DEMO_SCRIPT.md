...
# ▶ 3-MINUTE CUT  *(~630 words ≈ 3:00–3:15 at a brisk, confident pace)*
...
"American importers pay over eighty billion a year in tariffs, and more than two billion in refunds they're legally owed go unclaimed — claiming them is complicated, expensive, and bound by deadlines that quietly expire. Classification is legal reasoning, not lookup — forty-two percent of customs penalties come from a wrong code. And only fourteen thousand licensed brokers serve millions of importers — the expertise can't scale. The constraint I designed around: you can't legally remove the licensed broker. So the AI does the work, and the broker certifies it."

**[SHOW: Methodology — accuracy table + agent pipeline]**  *(Q2: research + method)*
"I embedded the entire U.S. tariff schedule and tens of thousands of CBP rulings into a vector database, so the classifier reasons over the actual law. I built a gold-standard benchmark to grade the pipeline — every classification must cite a real ruling, for one-hundred-percent citation grounding by construction. The duty math runs on a separate deterministic engine — base rate, Section 301, 232, fees — so dollar figures are always exact and auditable."

**[SHOW: Workflow auto-pilot → Broker queue: approve a filing, expand the duty stack → Coordination shipment timeline]**  *(Q2: agents)*
"On top sits a system of ten specialized agents — extraction, classification, duty calculation, refund-finding, sourcing — each a tool-using model call with structured output and retries. The hardest codes go through an adversarial debate — advocate, challenger, judge — and a verifier cross-checks the answer against live CBP rulings. The lifecycle runs autonomously: with zero clicks, it's drafted the security filing and the customs entry, routed them to a licensed broker for one approval, and posted the shipment into a coordination view that keeps importer, broker, and freight aligned on one timeline."

**[SHOW: Policy Lab reroute → Catalog landed-cost map → Factory Finder → Reg Watch]**  *(intelligence layer)*
"And I went beyond filing — when tariff rules change every month, classification alone isn't enough. The Policy Lab simulates any tariff shock across a catalog. The sourcing engine maps where production could move. The Factory Finder profiles specific factories — certifications, capacity, openness to new clients. Reg Watch monitors the Federal Register so new exclusions reach you the day they publish. That's what most customs tools don't try."

**[SHOW: Find-refunds → Deadlines → Command center]**  *(Q3)*
"Who's it for? Not just small Amazon sellers — that's where I'd start, but the honest target is anyone importing into the U.S. without a Fortune-500-grade trade compliance team — which is almost everyone. A nine-figure manufacturer hits the same tariff math as a DTC brand. What I built gives them what those internal teams have: a classifier they can trust, a refund finder that catches what their broker missed, a deadline tracker so nothing recoverable expires, and a strategic layer for sourcing and pricing when policy moves."

**[SHOW: Copilot answering a question]**  *(Q4 + close)*
"What I'd build next. Engineering: a CI gate so no change ships unless it beats the benchmark; model routing — cheap models for the easy classifications, frontier for the hard ones; and ACE Importer Portal access so the audit is self-serve — connect once, and the refund finder runs on your last two years automatically. The bigger ambition is provably correct classification. Today every classification already cites a real ruling and is cross-checked against live CBP precedent — that's the foundation. Next: go from cited precedent to a machine-checkable **proof** that the *legal procedure itself* was followed, attached to every entry — flipping compliance from something you defend *after* an audit to something you verify *before* you file. The General Rules of Interpretation are already a formal legal decision procedure. Start with one chapter — Chapter 85, electronics — encode its chapter notes and GRI 1 through 6 as a symbolic checker the LLM must satisfy, measure the accuracy lift, scale chapter by chapter. That turns 'reasonable care' from a vibe into a verifiable artifact. At filing scale, close the loop with real CBP outcomes — what the broker accepted, what customs liquidated unchanged — and train on that signal directly. You can't build it today — the law is deliberately ambiguous, and outcome data only exists after years of filings. The work of a customs broker, done by AI, certified by a human. That's Customs Agent Suite."
