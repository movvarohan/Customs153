...
# ▶ 3-MINUTE CUT  *(~565 words ≈ 3:05–3:15 at a confident pace)*
...
"Every year, American importers pay over eighty billion dollars in tariffs, and over two billion dollars in refunds they're owed go unclaimed — because claiming it is complicated, expensive, and bound by deadlines that quietly expire. I built Customs Agent Suite to fix two structural problems. First, classification is real legal reasoning, and forty-two percent of customs penalties come from a wrong code. Second, only about fourteen thousand licensed brokers serve millions of importers — the expertise can't scale. My insight: you can't legally remove the broker, so I let the AI do the work and have the broker certify it."

**[SHOW: Methodology — accuracy table + agent pipeline]**  *(Q2: research + architecture)*
"So how does it work? It's a retrieval system, not a model I trained. I embedded the entire U.S. tariff schedule and tens of thousands of CBP binding rulings into a vector database, so it reasons over real law, not memory. I built a gold-standard benchmark from real rulings, and every classification has to cite an actual one — that's one-hundred-percent citation grounding, by design. The duty math runs on a separate, deterministic engine, so the dollar figures are always exact and auditable."

**[SHOW: Workflow auto-pilot → Broker queue: approve a filing, expand the duty stack]**  *(Q2: agents)*
"On top sits a team of ten specialized agents. The hardest codes go through an adversarial debate — advocate, challenger, judge — then a verifier checks the answer against live customs rulings. The whole lifecycle runs on autopilot: with zero clicks, it's already drafted the security filing and the customs entry and handed them to a broker for one approval."

**[SHOW: Policy Lab reroute economics → Catalog landed-cost map → Factory Finder cards]**  *(the unique edge)*
"What really sets it apart: I didn't stop at filing — I built a strategic intelligence layer on top. The Policy Lab simulates any tariff change across a whole catalog in one click. The sourcing engine maps where production could move, and the Factory Finder vets specific factories — even which are taking new clients. A Reg Watch agent monitors the Federal Register so a new exclusion that saves you money reaches you first. Every other tool answers, how do I file this. I answer the bigger question: how do I protect my margins as the rules change."

**[SHOW: Find-refunds → Deadlines → Command center]**  *(Q3)*
"It's built for Amazon and direct-to-consumer brands importing from China — too small for a great broker, big enough to overpay. The wedge is a free historical audit: forward your last six months of entries and the refund finder reclassifies every line against current rules, surfaces what's recoverable, and flags the filing deadline before it expires. And every broker correction writes back into a per-customer SKU memory — so the same product is classified the same way every shipment. That data flywheel is the long-term moat."

**[SHOW: Copilot answering a question]**  *(Q4 + close)*
"From here, two directions. On the business side, I bring filing in-house with my own broker's license and automate drawback and export filings. On the technical side, the north star is classification that's *provable* and *self-improving*. Today the AI explains its reasoning; next, I want it to **prove** it — emit a step-by-step argument that it followed customs law, and attach that proof to the entry. That flips compliance from something you defend after an audit to something you verify before you file. And it learns from real CBP outcomes, so every shipment makes the next one sharper. I can't just build it today — that's the point: the law is deliberately ambiguous, and outcome data only accumulates once you're filing at scale, which is exactly what makes it defensible. The work of a customs broker, done by AI, certified by a human. That's Customs Agent Suite."
