...
"What I'd build next. Engineering: a CI gate so no change ships unless it beats the benchmark; model routing — cheap models for the easy classifications, frontier for the hard ones; and ACE Importer Portal access so the audit is self-serve — connect once, and the refund finder runs on your last two years automatically. The bigger ambition is **provably correct customs classification**. By that, I don't mean the AI eliminates all legal ambiguity. I mean every classification comes with a **machine-checkable record** showing which legal steps were followed — the product facts used, the tariff notes considered, the CBP rulings cited, and why competing codes were rejected. That turns 'reasonable care' from a vibe into a verifiable artifact attached to every entry — and unlike a moonshot, this is totally doable today. At filing scale, you close the loop with real CBP outcomes — what the broker accepted, what customs liquidated unchanged — and train on that signal directly. The work of a customs broker, done by AI, certified by a human. That's Customs Agent Suite."
