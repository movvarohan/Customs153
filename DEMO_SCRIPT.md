...
# ▶ 3-MINUTE CUT  *(~505 words — read at ~165 wpm)*

Written as flowing, plain sentences so it's easy to follow. SHOW cues are in **[brackets]** and aren't spoken. The fuller ~5-minute version is below if you want more detail.

**[SHOW: Home — hero + the "Why now" stats]**  *(Q1)*
"Every year, American importers pay over eighty billion dollars in tariffs, and billions of dollars in refunds they are legally owed go unclaimed, simply because no one is looking for them. We built Customs Agent Suite to fix three problems at once. The first is that classifying a product for customs is genuine legal reasoning, not a lookup, and forty-two percent of customs penalties come from getting that code wrong. The second is that there are only about fourteen thousand licensed customs brokers in the country, so the expertise can't possibly reach the millions of importers who need it. And the third is that the 2025 tariff changes left small importers with no way to keep up. Our key insight was that you can't legally remove the licensed broker from the process, so instead we let the AI do the work and have the broker certify it."

**[SHOW: Methodology — accuracy table + agent pipeline]**  *(Q2: research + architecture)*
"So how does it actually work? At its core, this is a retrieval system, not a model we trained ourselves. We embedded the entire U.S. tariff schedule and tens of thousands of binding customs rulings into a vector database, so the agent reasons over the real law instead of its own memory. To prove that it works, we built our own benchmark from real rulings and grade every change against it, and today every classification ships with a citation you can check. And because the duty math runs on a separate, deterministic engine with no language model involved, the dollar amounts are always exact and auditable."

**[SHOW: Workflow auto-pilot → Broker queue: approve a filing, expand the duty stack]**  *(Q2: agents)*
"On top of that sits a team of ten specialized agents. The hardest classifications are settled by an adversarial debate, where one agent argues for a code, another attacks it, and a third decides, and the result is checked against live customs rulings. The entire shipment lifecycle then runs on autopilot. As you can see here, with no clicks from anyone, the system has already drafted the security filing and the formal customs entry and handed them to a licensed broker for a single, informed approval."

**[SHOW: Policy Lab reroute economics → Catalog landed-cost map → Factory Finder cards]**  *(the unique edge)*
"But what really sets us apart is that we did not stop at filing paperwork. We built a strategic intelligence layer on top of it. The Policy Lab lets a CFO simulate any tariff change across their entire catalog in a single click. The sourcing engine researches where production could move and maps the true landed cost across countries. And the Factory Finder vets specific factories, telling you which ones are even taking new clients, and which are a short-term fix versus a long-term partner. Every other customs tool answers the question, how do I file this. We answer a much bigger one: how do I protect my margins as the rules change."

**[SHOW: Command center]**  *(Q3)*
"This is built for Amazon and direct-to-consumer brands importing from China, who are too small to afford a great broker but big enough to overpay badly. They recover five to fifteen percent of the duty they paid, they stop missing the deadlines that quietly erase refunds, and they finally get supply-chain intelligence that, until now, only the largest companies could afford."

**[SHOW: Copilot answering a question]**  *(Q4 + close)*
"From here, we go in two directions. On the business side, we bring filing fully in-house with our own broker's license, and we automate duty drawback and export filings. On the technical side, our north star is to make classification provably correct and self-improving. We want to build a neuro-symbolic engine that produces a machine-checkable proof for every code, which turns reasonable care from a judgment call into a verifiable artifact. And we want to train it on real customs outcomes — whether the broker accepted it, and whether customs cleared it without change — so the system genuinely learns to file better than a human over time. The work of a customs broker, done by AI and certified by a human. That is Customs Agent Suite."
