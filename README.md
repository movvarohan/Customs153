...
- **MPF / HMF — fee assumptions and limits.** Merchandise Processing Fee (FY2026 0.3464%, min $33.58 / max $651.50, source CBP Dec. 25-10) and Harbor Maintenance Fee (0.125% statutory) are entry-level value-based fees, computed once on the aggregate entered value — not per line. The PSC finder excludes both from the recoverable-amount calculation entirely (they're identical under the filed and proposed code on the same entry, so they cancel mathematically — see `src/core/agents/psc-finder.ts` and the smoke test verification noted in the commit messages). The duty calculator does NOT yet model **MPF exemptions for USMCA / FTA-qualifying goods** (those entries pay no MPF — broker should confirm FTA eligibility before filing). Mode of transport is now an explicit optional field on `HistoricalEntry` and `ExtractedShipment`; when absent we **assume ocean** for HMF and surface that assumption in the report instead of silently defaulting.
