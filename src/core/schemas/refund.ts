...
export const HistoricalEntry = z.object({
  ...
  /**
   * Named suppliers / shippers for this entry. Optional. When present the
   * risk-screener checks each name against OFAC SDN / BIS Entity List /
   * UFLPA, and any address against XUAR scrutiny regions. Importers who
   * don't track suppliers per entry can leave this empty; the screen will
   * still run on the importer name.
   */
...
export const HistoricalEntries = z.object({
  ...
  /** Importer EIN (Employer Identification Number) if known — used as a
   *  second tie-breaker in entity-graph matching when names are similar. */
...
export const PSCFindings = z.object({
  ...
  /** Risk & compliance profile generated by src/core/agents/risk-screener.ts.
   *  Optional so older PSCFindings payloads still validate. */
